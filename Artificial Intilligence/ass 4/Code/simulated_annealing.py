        ...
        for i in range(1,9):
            ...
            for j in range(1,9):
                if(L[i-1] != str(j)):
                    ...
                    y = [idcount,'s',x,hval]
                    states.append(y)
                    if(divideBy(counter) != 0 and max_val < hval):
                        uphills.append(idcount)
                        stuck = False
                    elif(divideBy(counter) == 0 and max_val > hval):
                        uphills.append(idcount)
                        stuck = False
...
def checkall(stuck):
    ...
    if max_val >= threshold:
        ...
        print('Stuckup!\n ')
    elif max_val < threshold and stuck == False :
        if divideBy(counter) != 0:
            print('Selected Uphill: '+str(states[max_id - 1]))
            counter += 1
            genarate_successor(states[max_id - 1][2])
        else:
            print('Selected Downhill: '+str(states[max_id - 1]))
            counter += 1
            genarate_successor(states[max_id - 1][2])
