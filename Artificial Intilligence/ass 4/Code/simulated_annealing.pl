...
checkall:- incr_c, divideBy(X1),not(X1=0), state(_,c,_,V),state(I1,s,L,V1),V1>V, retractall(state(_,_,_,_)),
	X is random(2)+1, X>1 ,assert(list_st([L])), write('Random Uphill Successor : ') ,write_list([I1,' ',L,' ',V1]),nl,  gnrt_sucsr(L), !.

checkall:- divideBy(X1), X1=0 , state(_,c,_,V),state(I1,s,L,V1), V1<V, retractall(state(_,_,_,_)),
	X is random(2)+1, X>1 ,assert(list_st([L])), write('Random Downhill Successor : ') ,write_list([I1,' ',L,' ',V1]),nl,  gnrt_sucsr(L), !.
...
incr_c:- restrt_cntr(V), V1 is V+1, retract(restrt_cntr(_)),
	assert(restrt_cntr(V1)).
divideBy(X1) :- restrt_cntr(V), X1 is V mod 5,!.
